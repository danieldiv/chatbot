elemento(chave1, valor1).
elemento(chave2, valor2).
elemento(chave3, valor3).

imprimir_valores :-
    elemento(_, Valor), % Usa uma variável anônima para ignorar a chave
    writeln(Valor),
    fail. % Força o backtracking para encontrar mais soluções

imprimir_valores. % Ponto de parada da recursão

:- initialization(imprimir_valores).

cumprimento("Olá").
cumprimento("Oi").
cumprimento("Oi, como vai?").
cumprimento("Bom dia").
...
main :-
    writeln("Digite uma opção de cumprimento:"),
    read(Opcao),
    resposta(Opcao, Resposta),
    writeln(Resposta),
    halt.
